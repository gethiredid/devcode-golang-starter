# Devcode Starter using Golang and Mux Level 5
...
3. Tambahkan validasi pada data yang akan dijadikan request body di route POST, PUT, DELETE pada file `controller.go`, untuk detail requirement validasinya dapat dilihat [disini](#detail-validasi-untuk-request-payload)
...
Anda dapat membuat table migrasi dengan menggunakan `struct`, contohnya ada pada bagian `type Contact struct {...}` di file `controller.go`.
